...
const FloatingChatbot = () => {
  ...
  return (
    <div className="fixed bottom-0 right-0 z-50 md:bottom-8 md:right-8">
      ...
      {/* Expanded Chatbox */}
      {isChatOpen && (
        <div className="flex flex-col w-full h-screen md:h-[80vh] md:w-[600px] bg-gray-800 md:rounded-2xl shadow-xl transform duration-300 ease-in-out">
          {/* Header */}
          <div className="bg-gradient-to-r from-purple-500 to-blue-500 text-white p-4 md:p-6 flex justify-between items-center md:rounded-t-2xl">
            ...
            {chat.map((message, index) => (
              <div key={index}>
                ...
                <div className="flex justify-end mb-3 md:mb-4">
                  <div className="bg-blue-600 text-white p-3 md:p-5 rounded-lg max-w-[85%] text-base md:text-lg shadow-md transform transition-transform duration-300 ease-in-out">
                    {message.userInput}
                  </div>
                </div>
                {/* Bot Response */}
          ...
          {/* Input Section */}
          <div className="p-4 md:p-6 bg-gray-800 flex items-center space-x-3 md:space-x-6">
            <input
              type="text"
              value={userMessage}
